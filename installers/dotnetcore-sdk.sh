...
[[ "$ARCH" == "amd64" ]] && ARCH=${ARCH/amd/x}
[[ "$ARCH" == "armhf" ]] && ARCH=${ARCH/hf/32}
...
for sdk in $sortedSdks; do
  url="https://dotnetcli.blob.core.windows.net/dotnet/Sdk/$sdk/dotnet-sdk-$sdk-linux-$ARCH.tar.gz"
  echo "$url" >>urls
...
parallel -a urls -j 16 "echo Downloading {} && wget -q {}"
